...
The backend may use RAG and tour tools before streaming the final assistant response. Intent classification, tour ranking, tour selection, authoritative date/availability checks, transfer estimates, pricing, discounts, participant-count interpretation, and reservation creation are backend responsibilities. The current UI displays the final assistant text, renders `TourRecommendationCards` only from validated `meta.tourRecommendation`, renders structured `uiAction` and `uiActions` controls, validates a chosen date against `availableDates` before sending it, renders `BirdMediaCard` from `meta.birdMatches` when bird profile media is present, and renders `ReservationConfirmationCard` from reservation metadata when present. Recommendation card fields are never parsed from assistant prose. If bird media fields are relative object keys, the UI resolves them through the backend media endpoint before rendering.
